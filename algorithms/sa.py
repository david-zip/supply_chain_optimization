...
class Parallelized_Simulated_Annealing():
    ...
    def __init__(self, model, env, **kwargs):
        ...
        self.Ti0        = [self.args['temp'][0] for _ in range(self.population)]
        self.Tf0        = [self.args['temp'][1] for _ in range(self.population)]
        self.T0         = [self.args['temp'][0] for _ in range(self.population)]
        self.eps        = [1 - (self.Tf0/self.Ti0)**(self.maxiter**(-1)) for _ in range(self.population)]

        # initialise list for algorithm
        self.parameters         = [self.params for _ in range(self.population)] 
        self.best_rewards       = [[] for _ in range(self.population)]                # best reward after every episode
    ...
    def _neighbourhood_search(self, i):
        ...
        for key, value in self.parameters[i].items():
            self.parameters[i][key] = torch.rand(value.shape) * (self.ub - self.lb) + self.lb
    ...
    def _find_best(self, i):
        ...
        """
        # identify best solution
        if self.current_reward[i] > self.best_value:
            self.best_parameters = copy.deepcopy(self.parameters[i])
            self.best_value      = self.current_reward[i]
        else:
            # metropolis acceptance probability
            r = np.random.uniform()
            if r > np.exp((self.best_value - self.current_reward[i])/self.T[i]):
                self.best_parameters = copy.deepcopy(self.parameters[i])
                self.best_value      = self.current_reward[i]
    ...
    def _cooling_schedule(self, i):
        ...
        Geometric cooling schedule
        """
        self.T[i] *= (1 - self.eps[i])
    
    def _run_parallel(self, i):
        """
        Run algorothm task in parallel
        """
