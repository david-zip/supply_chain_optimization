...
def evaluate(**kwargs):
    ...
    if kwargs['io'] == "mimo":
        ...
        SC_params_ = {'echelon_storage_cost':(5/2,10/2), 'echelon_storage_cap' :(20,7),
                        ...
                        'material_cost':{0:12, 1:13}, 'product_cost':{0:100, 1:300}}
    ...
    order_k                        = order_k.reshape((len(SC_model.SC_params['product_cost']), 
                                                        SC_model.n_echelons))

    backlog = np.array([0 for _ in range(len(SC_model.SC_params['product_cost']))]) # no backlog initially
    backlog = backlog.reshape(len(SC_model.SC_params['product_cost']), 1)
    # === SC run === #
    for step_k in range(steps_tot):
        d_k_                           = np.array([random_uniform_demand_si(demand_lb, demand_ub) \
                                            for _ in range(len(SC_model.SC_params['product_cost']))])
        d_k_                           = d_k_.reshape(len(SC_model.SC_params['product_cost']), 1)
        demand_history[step_k]         = d_k_[0,0]
        d_k                            = d_k_ + backlog
        sale_product, r_k, backlog     = SC_model.advance_supply_chain_orders_DE(order_k, d_k)
        reward_history[step_k]         = r_k[0,0]
        backlog_history[step_k]        = backlog[0,0]
        demand_backlog_history[step_k] = d_k[0,0]
        ...
        reward_bakclog[step_k]         = np.sum(SC_model.r_bakclog)
        orders_history[:,step_k]       = np.sum(order_k)
        warehouse_history[:,step_k]    = SC_model.warehouses[0]
